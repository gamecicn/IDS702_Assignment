...
$$
  bwt.oz = \beta_{1}*smoke_{i} + \beta_{2}*mht_{i} + \beta_{3}*mrace_{i} + \beta_{4}*mpregwt_{i} + \beta_{5}*parity_{i} + \beta_{0} + \varepsilon_{i}
$$
...
- R Square is : `r summary(final_model)$r.squared`
- Model p-value is : `r model_pvalue`

**Cofidence Itervals (95%) for each slope:**

```{r echo = FALSE, results='asis'}
xtable(confint(final_model))
```
...
plot(final_model, which = 3, col=c("blue4"))
plot(final_model, which = 4, col=c("blue4"))
...
- No apparent interaction was found between selected variables. 
...
- The smoke behavior can significantly decrease birth weight. 95% CI is (confint(final_model)[2,1], confint(final_model)[2,2])

- For some races, smoking could bring down birth weight more significantly than other races. 
...
1. The final model's R-Square value is only 0.15, which means the model cannot explain the response value well.
