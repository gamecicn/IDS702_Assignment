...
The following covariates of this dataset are not balanced because the absolute value of difference for this variables are bigger than 0.1 .
...
In this modle pg1's p value is `r summary(m6)$coefficients['pg1',][4][[1]]` which means
it is statistical significant and its coefficient is `r summary(m6)$coefficients['pg1',][1][[1]]` which means assigned to treatment group 1 
would increase the odds of patient's satisfaction status by `r exp(summary(m6)$coefficients['pg1',][1][[1]])*100 - 100`%. The AIC of this model is `r AIC(m6)`.
...
diff_mean <- mean((as.numeric(matchedrep_df$i_aqoc[matchedrep_df$pg==0]) - 1)/n) - 
                mean((as.numeric(matchedrep_df$i_aqoc[matchedrep_df$pg==1]) - 1)/n)
...
With the new matching policy, the balance table is :
 
```{r echo = FALSE, include=TRUE, results='asis', message=FALSE, warning=FALSE}
xtable(matchedrep_asd$Balance[abs(matchedrep_asd$Balance$Diff.Un) > 0.1, ])
```

The SE is `r se` and the 95% confidence interval is [`r diff_ci`].

 ```{r echo = FALSE, include=TRUE, results='asis', message=FALSE, warning=FALSE, fig.cap = "Coefficient of regression model"}
xtable(summary(m7))
``` 
The AIC of this model is `r AIC(m7)`.
...
1. The blance of matching data by one-to-one method is better, cause its every item's diff value
   is smaller than the corespondent value of one-to-many
   
2. The regression model's AIC of one-to-one method is smaller than the AIC of one-to-many.
   (`r AIC(m6)` VS `r AIC(m7)`)
